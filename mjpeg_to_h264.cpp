...
void MJPEGDecoder::init(int width, int height)
{
    if (disable_hw_codec_) {
        // Initialize CPU mjpeg decoder
        codec_ = avcodec_find_decoder(AV_CODEC_ID_MJPEG);
        if (!codec_)
            throw std::runtime_error("CPU mjpeg decoder not found");

        codec_ctx_ = avcodec_alloc_context3(codec_);
        if (avcodec_open2(codec_ctx_, codec_, nullptr) < 0)
            throw std::runtime_error("Failed to open CPU mjpeg decoder");

        // Setup CPU frame
        frame_cpu_->format = AV_PIX_FMT_YUVJ420P;  // CPU decoder output format
...
{
    if (disable_hw_codec_) {
        throw std::runtime_error("decode_2drm is not supported when hardware codec is disabled. Use decode_2cpu instead.");
    }
...
{
    if (disable_hw_codec_) {
        // Use CPU mjpeg decoder
        packet_->data = const_cast<uint8_t*>(jpeg_data);
        packet_->size = data_size;
        int ret = avcodec_send_packet(codec_ctx_, packet_);
        if (ret != 0)
            throw std::runtime_error("avcodec_send_packet (CPU decoder)");

        ret = avcodec_receive_frame(codec_ctx_, frame_cpu_);
        if (ret != 0)
            throw std::runtime_error("avcodec_receive_frame");
        return frame_cpu_;
    }
...
void H264Encoder::init(int width, int height, int framerate, int bitrate, int min_bitrate, int max_bitrate)
{
    if (disable_hw_codec_) {
        // Initialize CPU h264 encoder (libx264)
        codec_ = avcodec_find_encoder_by_name("libx264");
        if (!codec_)
            throw std::runtime_error("libx264 encoder not found");

        codec_ctx_cpu_ = avcodec_alloc_context3(codec_);
        codec_ctx_cpu_->width = width;
        codec_ctx_cpu_->height = height;
        codec_ctx_cpu_->time_base = {1, framerate};
        codec_ctx_cpu_->framerate = {framerate, 1};
        codec_ctx_cpu_->pix_fmt = AV_PIX_FMT_YUV420P;  // libx264 default format
        codec_ctx_cpu_->bit_rate = bitrate;
        codec_ctx_cpu_->gop_size = framerate;
        codec_ctx_cpu_->max_b_frames = 0;
        codec_ctx_cpu_->profile = FF_PROFILE_H264_HIGH;
        codec_ctx_cpu_->level = 41;
        codec_ctx_cpu_->flags |= AV_CODEC_FLAG_LOW_DELAY;

        AVDictionary* opts = nullptr;
        av_dict_set(&opts, "preset", "ultrafast", 0);
        av_dict_set(&opts, "tune", "zerolatency", 0);

        if (avcodec_open2(codec_ctx_cpu_, codec_, &opts) < 0)
            throw std::runtime_error("Failed to open libx264 encoder");

        av_dict_free(&opts);

        // Setup CPU frame
        frame_cpu_->format = AV_PIX_FMT_YUV420P;
        frame_cpu_->width = width;
        frame_cpu_->height = height;
        frame_cpu_->pts = 0;
    } else {
